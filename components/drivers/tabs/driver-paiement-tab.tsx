...
const entryTypeConfig: Record<string, { label: string; color: string }> = {
  salary: { label: fr.drivers.payments.salary, color: "bg-blue-50 text-blue-700 border-blue-200" },
  mission: { label: fr.drivers.payments.mission, color: "bg-violet-50 text-violet-700 border-violet-200" },
  bonus: { label: fr.drivers.payments.bonus, color: "bg-emerald-50 text-emerald-700 border-emerald-200" },
    ...
    <div className="space-y-5">
      ...
      <section className="rounded-2xl border border-blue-100 bg-gradient-to-br from-blue-50 to-indigo-50/40 p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-wider text-blue-800">{fr.drivers.pricing.currentRate}</p>
            <p className="text-[11px] text-blue-600">{fr.drivers.pricing.validFrom} {formatDate(driver.currentRate.startDate ?? driver.createdAt)}</p>
          </div>
        ...
        {driver.paymentType === "monthly" ? (
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-sm text-slate-500">/ {fr.drivers.pricing.month}</span>
          </div>
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-sm text-slate-500">/ {fr.drivers.pricing.hour}</span>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-xl border border-white bg-white/70 p-3">
              <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">{fr.drivers.pricing.perMission}</p>
              <p className="mt-1 text-xl font-bold text-slate-900 tabular-nums">
                {formatMAD(driver.currentRate.pricePerMission ?? 0)}
              </p>
            </div>
            <div className="rounded-xl border border-white bg-white/70 p-3">
              <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">{fr.drivers.pricing.perHour}</p>
              <p className="mt-1 text-xl font-bold text-slate-900 tabular-nums">
                {formatMAD(driver.currentRate.pricePerHour ?? 0)}
              </p>
            </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <div className="flex items-center gap-1.5 text-slate-400 mb-2">
            ...
            <p className="text-[10px] font-semibold uppercase tracking-wider">{fr.drivers.kpis.totalPaid}</p>
          </div>
          <p className="text-xl font-bold text-emerald-700 tabular-nums">{formatMAD(driver.totalEarned)}</p>
          <p className="text-[11px] text-slate-400 mt-1">{fr.drivers.payments.allTime}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <div className="flex items-center gap-1.5 text-slate-400 mb-2">
            <TrendingUp className="h-3.5 w-3.5" />
            <p className="text-[10px] font-semibold uppercase tracking-wider">{fr.drivers.payments.payments}</p>
          </div>
          <p className="text-xl font-bold text-slate-900 tabular-nums">{driver.paymentHistory.length}</p>
          <p className="text-[11px] text-slate-400 mt-1">{fr.drivers.payments.entries}</p>
        </div>
      </div>
      ...
      {driver.rateHistory.length > 0 && (
        <section className="rounded-2xl border border-slate-200 bg-white">
          <div className="flex items-center gap-2 border-b border-slate-100 px-5 py-3">
            ...
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500">{fr.drivers.pricing.history}</h3>
          </div>
              ...
              <div key={rate.id} className="flex items-center justify-between py-3">
                <div>
                  <p className="text-sm font-semibold text-slate-900">
                    {rate.type === "monthly"
                      ? `${formatMAD(rate.monthlySalary ?? 0)} / ${fr.drivers.pricing.month}`
                      : rate.type === "hourly"
                        ? `${formatMAD(rate.pricePerHour ?? 0)} / ${fr.drivers.pricing.hour}`
                        : `${formatMAD(rate.pricePerMission ?? 0)} / ${fr.drivers.pricing.missionShort}`}
                  </p>
                ...
                <div className="text-right">
                  ...
                  <p className="text-[11px] text-slate-400">→ {rate.endDate ? formatDate(rate.endDate) : fr.drivers.pricing.current}</p>
                </div>
              </div>
            ))}
          </div>
