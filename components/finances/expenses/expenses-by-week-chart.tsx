...
import { expensesByWeek, formatMAD, type ExpenseRecord } from "@/lib/expenses-data"

function TooltipDot({ active, payload, label }: any) {
  if (!active || !payload || !payload.length) return null
  return (
    <div className="rounded-lg border border-slate-200 bg-white px-3 py-2 shadow-lg">
      <p className="text-[11px] font-medium uppercase tracking-wider text-slate-400">
        Semaine {label.replace("S", "")}
      </p>
      <p className="mt-0.5 text-sm font-semibold text-slate-900">{formatMAD(payload[0].value)}</p>
    </div>
  )
}

export function ExpensesByWeekChart({ records }: { records: ExpenseRecord[] }) {
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-900">Dépenses par semaine</h3>
            <p className="mt-0.5 text-xs text-slate-500">{formatMAD(total)} sur la période</p>
          </div>
        </div>
      </div>
