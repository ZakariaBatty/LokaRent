...
function messageKeyForError(error: unknown) {
  ...
  if (error.code === "VALIDATION_ERROR") {
    if (error.message === "CONTRACT_ALREADY_EXISTS")
      return "contracts.errors.alreadyExists";
    if (error.message === "CONTRACT_AMENDMENT_NOT_REQUIRED")
      return "contracts.errors.amendmentNotRequired";
    if (error.message === "CONTRACT_PRICING_SNAPSHOT_REQUIRED")
      return "contracts.errors.pricingSnapshotRequired";
    if (error.message === "CONTRACT_PRICING_SNAPSHOT_MISSING")
      return "contracts.errors.pricingSnapshotRequired";
    if (error.message === "CONTRACT_TEMPLATE_REQUIRED")
      return "contracts.errors.templateRequired";
    if (error.message === "CONTRACT_DEFAULT_TEMPLATE_NOT_CONFIGURED")
      return "contracts.errors.defaultTemplateNotConfigured";
    if (error.message === "CONTRACT_TEMPLATE_NOT_FOUND")
      return "contracts.errors.templateNotFound";
    if (error.message === "CONTRACT_TEMPLATE_VERSION_REQUIRED")
      return "contracts.errors.templateVersionRequired";
    if (error.message === "CONTRACT_TEMPLATE_VERSION_NOT_FOUND")
      return "contracts.errors.templateVersionNotFound";
    if (error.message === "CONTRACT_RESERVATION_STATUS_NOT_ALLOWED")
      return "contracts.errors.reservationStatusNotAllowed";
    if (error.message === "CONTRACT_RESERVATION_NOT_CONFIRMED")
      return "contracts.errors.reservationNotConfirmed";
    if (error.message === "CONTRACT_GENERATION_NOT_ALLOWED")
      return "contracts.errors.generationNotAllowed";
    if (error.message === "CONTRACT_INVALID_PICKUP_MILEAGE")
      return "contracts.errors.invalidPickupMileage";
    if (error.message === "CONTRACT_INVALID_STATUS_TRANSITION")
      return "contracts.errors.invalidStatusTransition";
    if (error.message === "CONTRACT_LIFECYCLE_CONFLICT")
      return "contracts.errors.lifecycleConflict";
    if (error.message === "CONTRACT_RETURN_MILEAGE_TOO_LOW")
      return "contracts.errors.returnMileageTooLow";
    if (error.message === "CONTRACT_IMMUTABLE")
      return "contracts.errors.immutable";
    if (error.message === "CONTRACT_DELETE_BLOCKED")
      return "contracts.errors.deleteBlocked";
...
function messageKeyForUploadError(error: unknown) {
  ...
  if (error.message === "UPLOAD_FILE_TOO_LARGE")
    return "contracts.upload.errors.fileTooLarge";
  if (error.message === "UPLOAD_UNSUPPORTED_FILE_TYPE")
    return "contracts.upload.errors.unsupportedFile";
  if (error.message === "UPLOAD_PROVIDER_NOT_CONFIGURED")
    return "contracts.upload.errors.providerNotConfigured";
...
export async function generateContractAction(
  ...
  if (!parsed.success)
    return { success: false, messageKey: "contracts.errors.validation" };
  try {
    const context = await getActionContext(PERMISSIONS.CONTRACTS_CREATE);
    console.log(
      "Generating contract with context:",
      context,
      "and data:",
      parsed.data,
    );
...
export async function getContractByReservationAction(input: unknown) {
  ...
  if (!parsed.success)
    return {
      ...
      messageKey: "contracts.errors.validation",
    };
  try {
    const context = await getActionContext(PERMISSIONS.CONTRACTS_VIEW);
    const contract = await getContractByReservationService({
      ...context,
...
export async function listContractsByReservationAction(input: unknown) {
  ...
  try {
    const context = await getActionContext(PERMISSIONS.CONTRACTS_VIEW);
    const contracts = await listContractsByReservationService({
      ...context,
      reservationId: parsed.data.reservationId,
    });
    return {
      success: true as const,
      contracts: contracts.map(mapContractToUi),
    };
  } catch (error) {
    return {
      success: false as const,
...
export async function upsertContractTemplateAction(
  ...
  try {
    const context = await getActionContext(
      PERMISSIONS.SETTINGS_CONTRACT_TEMPLATE_MANAGE,
    );
...
export async function listContractTemplatesAction() {
  try {
    ...
    const templates = await listContractTemplatesService({
      ...context,
...
export async function createContractInspectionItemAction(
  input: unknown,
): Promise<ContractActionResult> {
...
export async function updateContractInspectionItemAction(
  input: unknown,
): Promise<ContractActionResult> {
...
export async function createContractSignatureAction(
  input: unknown,
): Promise<ContractActionResult> {
...
export async function listContractDocumentsAction(input: unknown) {
  ...
  if (!parsed.success)
    return {
      success: false as const,
      messageKey: "contracts.upload.errors.validation",
    };
  try {
    const context = await getActionContext(PERMISSIONS.CONTRACTS_VIEW);
...
export async function uploadContractDocumentAction(
  ...
  if (!parsed.success || !(file instanceof File))
    return { success: false, messageKey: "contracts.upload.errors.validation" };
  try {
    const context = await getActionContext(PERMISSIONS.CONTRACTS_CREATE);
...
export async function deleteContractDocumentAction(
  ...
  const parsed = contractIdSchema
    .extend({ documentId: contractIdSchema.shape.contractId })
    .safeParse(input);
  if (!parsed.success)
    return { success: false, messageKey: "contracts.upload.errors.validation" };
  try {
