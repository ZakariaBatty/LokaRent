...
export function mapInvoiceToUi(invoice: InvoiceWithDetails): Invoice {
  ...
  return {
    ...
    settlementWarning: paid + credited > total,
    ...
    creditNotes: invoice.creditNotesAsOriginal.map((note) => ({
      id: note.id,
      code: note.code,
      amount: toNumber(note.amount),
      currency: note.currency,
      reason: note.reason ?? undefined,
      issuedAt: toIsoDate(note.issuedAt),
      replacementInvoiceId: note.replacementInvoiceId ?? undefined,
    })),
